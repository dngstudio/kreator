<x-app-layout>
    ...
    <div x-data="{ count: {{ $user->subscriptions()->count() }} }">
    ...
    <div class="mb-6">
        @if($user->profile_image)
            <img src="{{ asset('storage/' . $user->profile_image) }}" alt="{{ $user->name }}'s Profile Image" class="w-32 h-32 rounded-full">
        @else
            <img src="{{ asset('images/default-profile.png') }}" alt="{{ $user->name }}'s Default Profile Image" class="w-32 h-32 rounded-full">
        @endif
    </div>
    ...
    @if(Auth::check() && Auth::user()->isAn('subscriber'))
        ...
        @else
            <a href="{{ route('unsubscribe.page', $user->id) }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">Unsubscribe</a>
        @endif
    @endif
